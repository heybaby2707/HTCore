        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            switch (dummySpell->Id)
            {
                ...
                case 91986:
                {
                    ...
                    int32 maxdamage = int32(SpellBaseDamageBonusDone(SPELL_SCHOOL_MASK_ALL) * multiplier + dummySpell->Effects[EFFECT_1].CalcValue(this)) / emberDot->GetMaxTicks();
        ...
        case SPELLFAMILY_ROGUE:
        {
            switch (dummySpell->Id)
            {
                case 76838: // Strikes of opportunity
                {
                    ...
                    // Don't let it proc from sweeping strikes, causes a cash
...
bool Unit::HandleProcTriggerSpell(Unit* victim, uint32 damage, uint32 absorb, AuraEffect* triggeredByAura, SpellInfo const* procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (trigger_spell_id)
    {
        ...
        case 50227:
        {
            ...
            if (GetTypeId() == TYPEID_PLAYER)
                ToPlayer()->RemoveSpellCategoryCooldown(23922, true);
            break;
        }
        // Maelstrom Weapon
        case 53817:
...
void Unit::UpdateSpeed(UnitMoveType mtype, bool forced)
{
	...
	if (sWorld->getIntConfig(CONFIG_DOUBLE_MOVING) == 1)
	{
		...
		{
 	 		if (zone && !(zone->flags & AREA_FLAG_WINTERGRASP))
 	 		{
 		 		movement_inc = 2;
 			} else {
	 	 		movement_inc = 1;
